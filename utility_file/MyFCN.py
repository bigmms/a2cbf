...
class DenseBlock(chainer.Chain):
    def __init__(self, ch, activation=F.relu):
        self.activation = activation
        super(DenseBlock, self).__init__(conv1=L.Convolution2D(ch, 64, 3, stride=1, pad=1, nobias=False),
                                         conv2=L.Convolution2D(ch + 64, 64, 3, stride=1, pad=1, nobias=False),
                                         conv3=L.Convolution2D((ch + 64) * 2, 64, 3, stride=1, pad=1, nobias=False),
                                         conv4=L.Convolution2D((ch + 64) * 4, 64, 3, stride=1, pad=1, nobias=False),
                                         conv5=L.Convolution2D((ch + 64) * 8, 64, 3, stride=1, pad=1, nobias=False))

    def __call__(self, input):
        x1 = F.leaky_relu(self.conv1(input))
        x1 = F.concat((input, x1), axis=1)

        x2 = F.leaky_relu(self.conv2(x1))
        x2 = F.concat((input, x1, x2), axis=1)

        x3 = F.leaky_relu(self.conv3(x2))
        x3 = F.concat((input, x1, x2, x3), axis=1)

        x4 = F.leaky_relu(self.conv4(x3))
        x4 = F.concat((input, x1, x2, x3, x4), axis=1)
