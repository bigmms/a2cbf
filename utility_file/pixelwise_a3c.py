...
class PixelWiseA3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def __init__(self, my_gpu, model, optimizer, t_max, gamma, beta=1e-2,
        ...
        self.shared_model.to_gpu(device=self.my_gpu)
    ...
    def update(self, statevar):
        ...
        if statevar is None:
            ...
            R = F.cast(vout.data, 'float32')  # 轉換為float32 type

        pi_loss = 0
        v_loss = 0
        for i in reversed(range(self.t_start, self.t)):
            R *= self.gamma            # eq1
            R += self.past_rewards[i]  # eq1
...
##############################
            pi_loss -= log_prob * F.cast(advantage.data, 'float32')  # eq4
...
##############################
        ...
        if self.keep_loss_scale_same and self.t - self.t_start < self.t_max:  # false
            factor = self.t_max / (self.t - self.t_start)
            pi_loss *= factor
            v_loss *= factor

        if self.normalize_grad_by_t_max:  # false
        ...
        self.model.cleargrads()  # 初始化 gradients
        total_loss.backward()
        # Copy the gradients to the globally shared model
        self.shared_model.cleargrads()  # 初始化 gradients
        ...
        self.optimizer.update(device=self.my_gpu)  # 更新網路
    ...
    def act_and_train(self, state, reward):
        ...
        statevar = chainer.backend.cuda.to_gpu(state, device=self.my_gpu)

        #self.past_rewards[self.t - 1] = reward
        self.past_rewards[self.t - 1] = chainer.backend.cuda.to_gpu(reward, device=self.my_gpu)
...
############################# for testing
    def act(self, obs):
        ...
        with chainer.no_backprop_mode():
            ...
            statevar = chainer.backend.cuda.to_gpu(obs, device=self.my_gpu)
            pout, _ = self.model.pi_and_v(statevar)
            if self.act_deterministically:
                #return pout.most_probable.data[0]
                return chainer.backend.cuda.to_cpu(pout.most_probable.data)
            else:
                #return pout.sample().data[0]
                return chainer.backend.cuda.to_cpu(pout.sample().data)
#########################
